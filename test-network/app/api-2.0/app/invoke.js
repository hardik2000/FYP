...
const invokeTransaction = async (channelName, chaincodeName, fcn, args, username, org_name, transientData) => {
    try {
        ...
        if (fcn === "invoke") {
            ...
            console.log(result)
            message = `Successfully queried the asset with key ${args[0]}`

        } else if (fcn === "history") {
            console.log(args[0])
            result = await contract.submitTransaction(fcn, args[0]);
            console.log(result)
            message = `Successfully queried history for the asset with key ${args[0]}`

        } else if (fcn === "delete") {
            console.log(args[0])
            result = await contract.submitTransaction(fcn, args[0]);
            console.log(result)
            message = `Successfully deleted the asset with key ${args[0]}`
